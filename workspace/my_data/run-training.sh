...
tmux new-session \; \
  ...
  split-window -h \; \
  send-keys 'source '$env_name'/bin/activate' C-m \; \
  send-keys 'CUDA_VISIBLE_DEVICES="-1" catalyst-rl run-trainer --config '$config C-m \; \
  split-window -h -t 0 \; \
  send-keys 'source '$env_name'/bin/activate' C-m \; \
  send-keys 'sleep 20s' C-m \; \
  send-keys 'CUDA_VISIBLE_DEVICES="-1" catalyst-rl run-samplers --seed '$seed' --config '$config C-m \; \
